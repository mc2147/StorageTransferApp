...
import {Grid, Image, Header, Segment, Form, Button, Checkbox, Message} from 'semantic-ui-react';
...
class App extends Component {
    ...
    async componentDidMount() {
        ...
        fileList1, fileList2, folderList1, folderList2} = authStatusResponse.data;
    ...
    unlinkAccount(accountNum) {
        if (accountNum == 1) {
            axios.put('/api/unlink-1').then(response => {
                this.setState({
                    auth1:false,
                    provider1:'',
                    accountInfo1:{}
    ...
    render() {
        ...
        console.log("this.state: ", this.state);
        const {provider1, provider2, auth1, auth2, accountInfo1, accountInfo2,
        fileList1, folderList1, fileList2, folderList2,
        customDestinationPath} = this.state;
                ...
                <div className="ui grid">
                  <div className="row">
                        ...
                        <Segment>
                            ...
                            {auth2 ? (
                                ...
                                <Message positive>
                                    ...
                                    <b>Your Account Information</b>:<br/>
                                    Provider: {provider1}<br/>
                                    Name: {accountInfo1.name} test<br/>
                                    Email: {accountInfo1.email}<br/>
                                    <br/>
                                    </p>
                                </Message>                                
                    ...
                    <Grid.Row columns={1} style={{padding:0}}>
                         ...
                         <Form onSubmit={this.handleSubmit} textAlign='center'>
                            ...
                            <Message negative>
                                ...
                                style={{marginTop:"5px"}}/>
                                <Form.Field>
                                <label>Select Source File or Folder</label>
                                {(this.state.sourceAccount == 1) ? 
                                    (
                                        <select>
                                        ...
                                        {fileList1.map((file) =>
                                            <option key={file.tag} value={file.tag}>{file.name}</option>
                                        )}
                                        </select>                                                
                                    )
                                    :
                            ...
                            <Segment.Group horizontal style={{margin:0}}>
                                <Segment>
                                    <Form.Field>
                                    <label>Select Destination File or Folder</label>
                                    <select style={{marginBottom:'5px'}} disabled={customDestinationPath != ''}>
                                        {folderList1.map((folder) =>
                                            <option key={folder.tag} value={folder.tag}>{folder.name} [Folder]</option>
                                        )}
                                        {fileList1.map((file) =>
                            ...
                            <Button type='submit' className="center" positive style={{margin:'auto'}}>Transfer File</Button>                                
                            </Segment>
                        </Segment.Group>                            
                        </Grid.Column>
                    </Grid.Row>
